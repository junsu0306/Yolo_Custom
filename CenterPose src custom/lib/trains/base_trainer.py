...
from lib.pruning.dlasg_pruning import dlasg_blockwise_pruning
from lib.pruning.memory_usage import measure_model_memory, measure_pruned_layer_memory, custom_memory_loss_function
import cv2
...
class BaseTrainer(object):
    def __init__(
        ...
        self.file_stream = open('./memmory_comparison.txt', 'a', encoding='utf-8')
        self.model_with_loss = ModelWithLoss(model, self.loss)
        self.dummy_input = {
                'input': torch.randn(1, 3, 512, 512, device=opt.device),
                'pre_img':torch.randn(1, 3, 512, 512, device=opt.device),
                'pre_hm': torch.randn(1, 1, 512, 512, device=opt.device),
                'pre_hm_hp': torch.randn(1, 8, 512, 512, device=opt.device)
            }
    ...
    def run_epoch(self, phase, epoch, data_loader):
        ...
        for iter_id, batch in enumerate(data_loader):
            ...
            if phase == 'train':
                ...
                dlasg_blockwise_pruning(model_for_pruning, sparsity=0.5, device = opt.device)# GPU 처리 
                
                if self.file_stream is not None:
                    _ = measure_model_memory(model_for_pruning, self.dummy_input, opt.device, self.file_stream) #reducing 전 메모리 측정

                mem_usg = measure_pruned_layer_memory(model_for_pruning, self.dummy_input, opt.device, self.file_stream) #reducing 후 메모리 측정 
                
                self.file_stream = None # Close the file stream after starting point
                model_for_pruning = model_for_pruning.to(opt.device)
    
                hyperparam = 1.0
                device_condition_memory = 1.0 #단위 조심
                
                loss += custom_memory_loss_function(mem_usg, hyperparam, device_condition_memory)
