...
class CustomDetector(ObjectPoseDetector):
    def __init__(self, opt):
        super(ObjectPoseDetector, self).__init__(opt)
    
    def change_model(self, new_model_path ="./reduced_model_50_shoe.pth"):
...
def demo(opt, meta):
    ...
    detector = CustomDetector(opt)
    #detector.change_model("./reduced_model_50_shoe.pth")
    #detector.change_model("./reduced_model_50_book.pth")# 이 부분은 리듀싱 이후 모델 바꿀 때 사용하는 부분
    ...
    if opt.demo == 'webcam' or \
        ...
        output_dir = "/workspace/MH/CenterPose/exp"
